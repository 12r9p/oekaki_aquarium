...
function StatusCard({
            ...
            <div className="w-full min-w-0">
                <div className="flex items-start justify-between gap-1.5 sm:gap-4">
                    <div className="min-w-0 flex-1">
                        <div className="text-[10px] sm:text-sm font-medium text-slate-500 break-words whitespace-normal leading-tight">{label}</div>
                        <div className="mt-1 sm:mt-2 text-2xl sm:text-3xl font-bold tracking-tight text-slate-900 break-all whitespace-normal leading-none">{value}</div>
                    </div>
                    <div className={`rounded-lg p-1.5 sm:p-2.5 flex-shrink-0 [&_svg]:h-4 [&_svg]:w-4 sm:[&_svg]:h-5 sm:[&_svg]:w-5 ${tones[tone]}`}>{icon}</div>
                </div>
                {progress !== undefined && (
                    <div className="mt-2.5 sm:mt-3.5 h-1.5 w-full rounded-full bg-slate-100 overflow-hidden">
                        <div className={`h-full ${progressColor} transition-all duration-500`} style={{ width: `${progress}%` }} />
                    </div>
                )}
            </div>
            <div className="mt-3 sm:mt-4 flex w-full flex-col items-start justify-between text-[10px] sm:text-xs text-slate-500 min-w-0 gap-1 sm:flex-row sm:items-center sm:gap-2">
                <span className="break-words text-left whitespace-normal leading-normal flex-1" title={detail}>{detail}</span>
                {onClick && <ExternalLink className="h-3 w-3 sm:h-3.5 sm:w-3.5 flex-shrink-0 text-slate-400 sm:text-slate-500" />}
            </div>
        </>
    );

    const baseClass = "rounded-xl border border-slate-200 bg-white p-3 sm:p-5 text-left shadow-sm flex flex-col justify-between";
    ...
    if (onClick) {
        return (
            <Button
                variant="ghost"
                onClick={onClick}
                className={`${baseClass} transition hover:border-sky-300 hover:shadow w-full h-auto text-left font-normal text-slate-900 hover:bg-white hover:text-slate-900 focus-visible:ring-sky-500 whitespace-normal min-w-0 p-3 sm:p-5`}
            >
                {content}
            </Button>
        );
    ...
    return (
        <div className={`${baseClass} min-w-0 w-full`}>
            {content}
        </div>
    );
}
                ...
                <section className="grid gap-5 lg:grid-cols-3">
                    ...
                    <div className="rounded-xl border border-slate-200 bg-white p-5 shadow-sm">
                        ...
                        <div className="mt-4 flex flex-col divide-y divide-slate-100">
                            <Button variant="ghost" className="flex items-center justify-between py-3 px-0 text-sm hover:text-sky-700 hover:bg-transparent h-auto font-normal text-slate-800 whitespace-normal min-w-0 w-full" onClick={() => onNavigate("pending")}>
                                <span className="break-words mr-2 text-left">承認待ちの魚</span><span className="font-bold flex-shrink-0">{pendingFish.length}匹</span>
                            </Button>
                            <Button variant="ghost" className="flex items-center justify-between py-3 px-0 text-sm hover:text-sky-700 hover:bg-transparent h-auto font-normal text-slate-800 whitespace-normal min-w-0 w-full" onClick={() => onNavigate("layout")}>
                                <span className="break-words mr-2 text-left">切断中のモニター</span><span className="font-bold flex-shrink-0">{disconnectedDisplays}台</span>
                            </Button>
                            <Button variant="ghost" className="flex items-center justify-between py-3 px-0 text-sm hover:text-sky-700 hover:bg-transparent h-auto font-normal text-slate-800 whitespace-normal min-w-0 w-full" onClick={() => onNavigate("fish")}>
                                <span className="break-words mr-2 text-left">非表示の魚</span><span className="font-bold flex-shrink-0">{activeFish.filter(fish => fish.isArchived).length}匹</span>
                            </Button>
                        </div>
                    </div>
